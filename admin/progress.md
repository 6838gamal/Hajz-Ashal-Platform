...
## ✅ لوحة تحكم مالك المنصة (Platform Admin) — مكتملة

- [x] موديول `modules/platform_admin` (Domain/Infra/Application/Presentation) + جدول `platform_admins` (migration `d0634e429453`)
- [x] تسجيل دخول منفصل بالكامل عن حسابات المنشآت، برمز JWT يحمل `scope: "platform_admin"`
- [x] إدارة المنشآت: عرض/بحث/تعديل (الاسم والحالة)/حذف (soft delete)
- [x] إدارة الفروع لكل منشأة: إضافة/تعديل/حذف
- [x] إدارة المستخدمين عبر كل المنشآت: إضافة/تعديل (الاسم، الحالة، كلمة المرور)/حذف
- [x] إدارة الأدوار والصلاحيات العامة (نطاق عابر للمنشآت): إنشاء/تعديل/حذف دور + تعيين/إزالة دور من مستخدم
- [x] واجهة أمامية مستقلة تحت `/admin/login` و `/admin` (قوالب في `admin/templates`, أصول في `admin/static`)
- [x] سكربت `scripts/create_platform_admin.py` لإنشاء/تصفير حساب مالك المنصة
- [x] توثيق كامل في `admin/README.md`
